...
class _AltinState extends State<Altin> {
  ...
  Future _istek() async {
    ...
    setState(() {});
    await http
        .get('https://bigpara.hurriyet.com.tr/altin/gram-altin-fiyati/')
        .then((_gelenveri) {
      ...
      _htmlveri1 = _gelenveri.body
          ...
          .replaceAll('\t', '') //tab olan yerleri siler
          .replaceAll('  ', '');
      RegExp arama = new RegExp('<span class="value up">(.*?)</span>');
      Match eslesen = arama.firstMatch(_htmlveri);
      _htmlveri = eslesen.group(1);

      RegExp arama1 = new RegExp('<span class="value dw">(.*?)</span>');
