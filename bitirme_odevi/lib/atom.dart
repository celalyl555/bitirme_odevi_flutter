...
class _AtomState extends State<Atom> {
  ...
  Future _istek() async {
    ...
    setState(() {});
    await http
        .get('https://tr.investing.com/crypto/cosmos/atom-try-chart')
        .then((_gelenveri) {
      ...
      _htmlveri1 = _gelenveri.body
          ...
          .replaceAll('  ', '');
      RegExp arama =
          new RegExp('<span class="inlineblock pid-1159025-ask">(.*?)</span>');
      Match eslesen = arama.firstMatch(_htmlveri);
      _htmlveri = eslesen.group(1);

      RegExp arama1 =
          new RegExp('<span class="inlineblock pid-1159025-bid">(.*?)</span>');
